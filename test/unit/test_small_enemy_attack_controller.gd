...
func test_line_1x4_includes_origin_cell() -> void:
    ...
    assert_eq(cells, [Vector2i(2, 2), Vector2i(3, 2), Vector2i(4, 2), Vector2i(5, 2)])
...
func test_surround_3x3_includes_origin_cell() -> void:
    var controller := SmallEnemyAttackController.new()
    controller.set_attack_pattern(SmallEnemyAttackController.AttackPattern.SURROUND_3X3)

    var cells := controller.get_attack_cells(Vector2i(2, 2), Vector2.UP)

...
Wave 清場
→ 短暫 wave gap
→ 三選一 reward（Major / Minor / 地形 / 未來壓力）
→ 效果立即套用
→ 下一波敵人數量與強度提升
→ 重複，直到玩家死亡
```
...
| Break Land           | 移除安全連通地，每 wave 上限拆 2 塊          | Aggressive reward 常見 downside，有明確每波上限 |
| Move Land            | 新增一塊 + 移除一塊，形狀改變                | 保留現有 random 邏輯                            |
...
> 待確認事項：Major 之間是否存在互斥或組合規則（例如 Chain Dash 與 Smash 同時存在時，一次 Dash 輸入該觸發哪一個）——見第 12 節。
...
移動 1 塊 land，但 -1 max HP
拆 1 塊安全 land，但 -0.2s Dash 冷卻
+普攻傷害，但下一波敵人生成點更靠近玩家
```
...
6. Aggressive reward 選滿後，地圖碎裂視覺明顯
```
...
- **地形碎裂下限保護**：每個 wave 最多允許 Break Land 拆除 2 塊地；每 5 波固定給的 Expand Land 從 1 塊提高到 10 塊。理論上長線是緩慢淨增長，不需要額外的「剩餘 land 過低禁用 Break」保護規則。
...
1. **Dash ability_override 的互斥規則**：Chain Dash 與 Smash 是否互斥（同一 build 只能存在其中一種 dash 型態），還是可以共存並疊加觸發順序？如果共存，一次 Dash 輸入的實際行為要如何決議？這會直接決定 `PlayerRunBuild.ability_overrides` 的資料形狀（單一 enum vs 疊加清單）。
2. **Guard Shredder / Execution 是否對 ModeEnemy elite 生效**：兩者已經是要主動選的 Major，風險比原本當 baseline 時低很多，但仍需決定 elite 是否要有「免疫瞬殺/瞬破」標記，以及畫面上要不要區分。
3. **哪些 TODO 條目要先升到 Active，對齊第 11 節的六項切片**：目前 TODO 的 Draft 條目（Infinite Chaos Wave Mode、Major/Minor Build、Guard Hit SFX、SmallEnemy Pattern Director 等）彼此都合理，但沒有標出哪幾條是宣傳切片的必要路徑，建議先把對應到第 11 節六項的部分拆出來、優先推進到 Active。
...
Terrain reward 套用 Break Land 每 wave 上限 2 塊、Expand Land 每 5 波 x10 塊的新數字
...
地形只要求 land 連通，允許隨 Aggressive reward 逐漸碎裂；Break Land 每 wave 上限 2 塊，Expand Land 每 5 波給 10 塊
...
| 術語                       | 說明                                                                                           |
| -------------------------- | ---------------------------------------------------------------------------------------------- |
| PlayerRunBuild             | 玩家 run 內累積的 Major/Minor 效果與觸發式行為容器                                             |
| Major Effect               | 上限 4 個，改變技能行為 / 規則（例如把 Dash 換成 Smash）                                       |
| Minor Effect               | 無上限，改數值或小規則，可無限疊加                                                             |
| Ability Override           | Build 中決定當前技能實際行為型態的欄位（例如 dash_type）                                       |
| Triggered Effect           | 掛在特定事件（on_dash_hit / on_kill / on_stagger）上的效果清單                                 |

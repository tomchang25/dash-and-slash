...
- Read `dev/skills/gdscript_abstract.md` before writing the `@abstract` base and `@abstract apply`. `PlayerStatEffect` stays `@abstract` because it inherits the still-abstract `apply`; only the leaves are concrete.

# Tick Arena Visual Readability 03: Small Enemy Pattern Director

Parent Plan: `tick_arena_visual_readability_and_identity.md`

## Goal

Explore making SmallEnemy the main spawn body and content multiplier through readable attack-pattern identities, after the runtime presenter and baseline SmallEnemy pose sheet give those variants enough visual language to stay clear.

## Summary

This child should come after the sprite runtime scaffold and baseline pose sheet. The direction is to add roughly six to eight SmallEnemy pattern identities through attack data and readable mark, tint, weapon, or silhouette-accent language while keeping SmallEnemy at about 60 percent or more of base spawns. This gives the game tactical variety without immediately adding many new enemy scenes.

Codebase context gathered so far suggests SmallEnemy already selects one attack profile from authored enemy data, and the shared attack data supports line, wide, square, and full-line cell shapes. The later implementation spec should verify whether additional identity metadata belongs in enemy data, spawn entries, separate scene variants, or the enemy visual presenter contract.

## Sketch

- Candidate shape: define a small set of named SmallEnemy pattern identities, each pairing one attack profile with a readable body mark, tint, weapon marker, or silhouette accent that can be displayed through the presenter.
...
- Current attack data can likely carry much of the mechanical variety through cell shape and tuning, but the later spec should verify whether pattern identity needs display metadata such as visual key, marker, or spawn label.
...
- A Comfy or prompt-config pipeline becomes worth considering here only if the baseline pose sheet has proven the production grammar and variant volume is the actual bottleneck.
- Candidate files to inspect at spec time: `game/entities/enemies/data/small_enemy.tres`, enemy attack data definitions, SmallEnemy scene/script, the enemy visual presenter, wave support spawn selection, and any spawn-planner assumptions about enemy scene identity.

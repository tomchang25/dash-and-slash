...
- Direction is the sign of each victim's offset from the Smash center, allowing all eight neighboring directions. This child assumes base Smash has already produced an open player landing before the Major's radial push pass; the separate occupied-landing Draft owns any zero-offset collision rule.
...
5. No base Smash resolution for an occupied locked landing; that non-cancelling result remains in its separate Draft until decided.

...
This child converts the reward artifact model to Resource-backed definitions and introduces `ArtifactRegistry` as the content catalog. `_make_default_artifacts()` is deleted outright. Production generator construction reads from a default registry Resource; tests can inject small synthetic registries. The generator remains a pure picker: read registry artifacts, filter by derived kind/min wave/eligibility, shuffle, return choices.
...
- `TickRunController` remains the production owner that constructs the generator. Its 02b cadence, `Minor x2` assembly, and forced curse confirmation must stay behaviorally unchanged.
...
- Update generator construction and tests to use registry injection/default registry loading.
...
| `game/tick_arena/run/tick_run_controller.gd`                                                                                                 | Small       | Construct the generator with the default registry Resource while preserving 02b flow.                          |
...
6. Update `TickRunController` and tests to construct the generator from the registry path/injection seam.
...
- The default registry path in this child is intentionally temporary if it lives under `game/tick_arena/reward/`; 02d owns moving it into the final domain-first data layout.

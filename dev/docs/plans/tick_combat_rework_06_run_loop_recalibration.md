...
4. Terrain cadence, reward gaps, death, and restart are integrated into the tick arena as run-loop behavior, with terrain mutation preserving connected land and reusing the existing production grid presentation path.
...
| 6e    | Terrain, death, and restart integration | Connect reward application, terrain mutation, wave gaps, death, restart, and end-to-end run reset in the tick arena.                                                     |
...
Terrain cadence preserves the current conceptual loop: one automatic terrain operation after each normal wave clear, milestone expansion by ten land cells, and connected land as the hard validity rule. The final operation mix can be tuned inside the terrain integration spec without changing this phase boundary.
...
1. A full tick-arena run plays from wave start through combat, reward choice, terrain cadence, milestone waves, death, and restart.
...
5. Terrain mutation updates the production grid presentation correctly and never breaks connected-land validity.

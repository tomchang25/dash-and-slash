...
2. ChargeEnemy must search a cardinal line up to five cells, wind up when the player is in that attack line, and resolve against the committed direction without retargeting during the windup.
...
5. DashEnemy must use a five-cell cardinal ambush: it commits to the cell one step beyond the player along its approach line, cancels if that landing cell is occupied before detonation, and attacks the locked player cell from the opposite side without relying on player combat facing.
...
Charge range is five cells. ChargeEnemy commits only when the player is aligned in its cardinal attack line and the enemy has completed its existing facing requirement. Windup locks the direction and five-cell terrain-bounded path.
...
DashEnemy searches a cardinal line up to five cells. When the player is found, the committed landing cell is one step beyond the player's locked cell along the direction from DashEnemy through the player; this is called the player's back side for this attack and does not read player facing.
...
1. ChargeEnemy presents a five-cell committed threat and resolves every crossed cell in order without overlapping logical occupancy.
...
5. DashEnemy visibly commits to a valid cell across the player, cancels when that landing becomes unavailable, and never retargets after windup.

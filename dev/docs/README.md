...
A plan too large for one change decomposes into child slices alongside it. Non-trivial children normally start as sketches (`<parent_scope>_<NN>_<slug>.sketch.md`) per `dev/foundation/core/workflows/sketch_standard.md`; skip the sketch only when the child boundary is small and obvious. Implementation always runs from a codebase-verified spec (`<parent_scope>_<NN>_<slug>.implementation_spec.md`) per `dev/foundation/core/workflows/implementation_spec_standard.md`. Children are pointed to from the parent's child overview table rather than `TODO.md`, and each child spec archives when it ships.

Probes are the exception to the no-`Status:` rule in `plans/`. They use `Status: Draft probe.` and `Decision:` metadata, may be problem-only, and do not need a TODO pointer while they are discussion handoffs. Use `dev/foundation/core/workflows/probe_standard.md` for this shape.

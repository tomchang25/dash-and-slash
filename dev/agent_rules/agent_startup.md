...
Command workflows live in `dev/foundation/core/workflows/commands/`, except project-local commands. When asked to do a command task, read the matching file before acting and follow it exactly. Slash form, dash form, `cmd <name>`, and natural-language requests are all valid: `/closeout`, `-closeout`, `cmd closeout`, and "close out this work" all mean the `closeout` command and must read `dev/foundation/core/workflows/commands/closeout.md`.

The opencode slash-command files under `.opencode/commands/` are thin entry wrappers only. They must not duplicate the workflow. The final source of truth remains `dev/foundation/core/workflows/commands/`.

Reviews use `dev/foundation/core/workflows/review_standard.md`; command files define the review scope, and the shared review standard defines depth, related-code search, reporting, and verdict language.

- `/closeout` -> `dev/foundation/core/workflows/commands/closeout.md`: closes out completed work — staged changes or a feature branch covering one or more plans (CHANGELOG + TODO + archive plans, optional commit-message suggestion only when explicitly asked).
- `/commit-msg` -> `dev/foundation/core/workflows/commands/commit-msg.md`: suggests a conventional commit message for currently staged changes without staging, committing, pushing, or opening a PR.
...
- `/pr-review` -> `dev/foundation/core/workflows/commands/pr-review.md`: reviews the branch against the base branch, then generates a PR title/description without creating files or opening a PR.
- `/stage-review` -> `dev/foundation/core/workflows/commands/stage-review.md`: checks staged changes against the plan spec and standards lint.
- `/research-context` -> `dev/foundation/core/workflows/commands/research-context.md`: retrieves relevant codebase context for an idea, scratchboard entry, draft, or small plan without implementing changes.
- `/spec-discuss` -> `dev/foundation/core/workflows/commands/spec-discuss.md`: inspects a focused target against live code, recommends resolutions for user-authority decisions, and locks the input for a later spec build without editing files.
- `/spec-build` -> `dev/foundation/core/workflows/commands/spec-build.md`: re-verifies live code, writes the final implementation spec, updates its parent or TODO lifecycle pointers, and lints the documentation without implementing code.

## Planning And Docs

When asked to build a plan, implementation spec, or sketch, follow the matching workflow in `dev/foundation/core/workflows/`: `plan_standard.md` for durable feature plans, `sketch_standard.md` for plan-child exploration, and `implementation_spec_standard.md` for the final codebase-verified handoff. "Spec" on its own always means an implementation spec. The normal larger-work flow is probe -> plan -> sketch -> implementation spec; skip the sketch only for small, obvious child boundaries. Narrow actionable work may go from a feature request or probe directly to a standalone implementation spec. Follow the plan lifecycle in `dev/docs/README.md` and `dev/standards/` for any relevant domain standard. Plans and standalone implementation documents go in `dev/docs/plans/` with a one-line pointer in `TODO.md`; a plan's child sketches/specs are pointed to from the parent plan's child overview table instead. When asked to capture an early problem observation, design tension, codebase discussion handoff, or unresolved architectural thought without a chosen implementation direction, use `dev/foundation/core/workflows/probe_standard.md`.
...
- **Runtime ownership**: read `dev/foundation/core/standards/runtime_ownership.md` before introducing or renaming Controllers, Systems, Stores, Services, save providers, or other runtime state owners.
- **Commits**: conventional commits format — read `dev/foundation/core/skills/conventional_commits.md` when writing commit messages. Do not hard-wrap prose.
- **Change summaries**: read `dev/foundation/core/standards/change_summary_standard.md` before writing commit messages, PR descriptions, CHANGELOG entries, closeout output, review summaries, or other completed-work summaries.
- **Probes**: read `dev/foundation/core/workflows/probe_standard.md` before creating early problem notes, design tension notes, architecture discussion handoffs, or codebase review conclusions that are not implementation plans.
- **Audio events**: use `AudioManager.play_event()` for gameplay, UI, and music playback; read `dev/foundation/platforms/godot/skills/audio_event_usage.md` before changing audio playback, SFX resources, music, or `AudioManager` call sites.
...
- **State machines**: the `StateMachine` + `State` framework is behavior-delegation, not a state-label holder; read `dev/foundation/platforms/godot/skills/state_machine_pattern.md` before changing entity states, FSM scene wiring, or transition logic. For grid enemies, tick runtime, attack/recovery timing, or capped facing, also read `dev/skills/state_machine_tick_grid_addendum.md`.
- **Scene routing**: `SceneRouter` owns production scene transitions; read `dev/foundation/platforms/godot/standards/scene_routing_standard.md` and `dev/foundation/platforms/godot/skills/scene_router_usage.md` before changing navigation.
- **Main Menu**: read `dev/standards/main_menu_standard.md` before editing `game/meta/main_menu/`.
- **Settings overlay**: `SettingsStore` owns `user://settings.json`; read `dev/foundation/platforms/godot/standards/settings_overlay_standard.md` and `dev/foundation/platforms/godot/skills/settings_overlay_usage.md` before adding settings.
- **Theme and UI styling**: read `dev/foundation/platforms/godot/standards/theme_standard.md` and `dev/foundation/platforms/godot/skills/godot4_theme_override.md` before changing fonts, StyleBoxes, theme type variations, or static UI colors.
- **Debug mode**: check `Debug.enabled`, not `OS.is_debug_build()` directly; read `dev/foundation/platforms/godot/standards/debug_standard.md` and `dev/foundation/platforms/godot/skills/debug_mode_usage.md` before adding debug behavior.
...
- **Error guards**: never use `assert()` for runtime guards. Use explicit `if` guards and read `dev/foundation/platforms/godot/standards/error_guard_standard.md` before adding or changing precondition checks, ToastManager channel selection, or bare `push_error` / `push_warning` call sites.
- **GDScript abstract APIs**: read `dev/foundation/platforms/godot/skills/gdscript_abstract.md` before introducing or changing `@abstract` classes or methods.
- **GDScript structure & naming (mandatory gate)**: before the first `Edit`/`Write` touching any `.gd` file in a session, read `dev/foundation/platforms/godot/standards/gdscript_structure_standard.md` and `dev/foundation/platforms/godot/standards/naming_conventions.md` in full. Do not gate this on judging whether the change "counts" as adding, renaming, or restructuring — any script touch qualifies, including pure logic edits that add or rename a single function. Persistent scene nodes and runtime `add_child()` exceptions follow `dev/foundation/platforms/godot/standards/scene_node_source_standard.md`; reusable component layout/preview rules follow `dev/foundation/platforms/godot/standards/component_scene_standard.md`. Node-source and no-`[connection]` rules are lint-enforced.
- **Versioning**: read `dev/foundation/core/skills/semantic_versioning.md` before release/version bump work or when a change needs public API version impact assessment.

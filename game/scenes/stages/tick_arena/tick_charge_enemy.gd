# tick_charge_enemy.gd
# Interim grey-box charger actor carried over from the prototype to keep the tick arena playable
...
func _detonate() -> void:
    ...
    for line_cell: Vector2i in _attack_tiles:
        if not _engine.is_cell_open_for_enemy(line_cell, self):
            break
        dest = line_cell
    if dest != cell:

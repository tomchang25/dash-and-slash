...
## Placeholder row-per-direction mapping for the scaffold sheet.
const FRAME_ROW_BY_DIRECTION := {
    Direction.DOWN: 0,
    Direction.LEFT: 1,
    Direction.RIGHT: 2,
    Direction.UP: 3,
...
func _apply_frame_coords() -> void:
    frame_coords = Vector2i(FRAME_COLUMN_BY_STATE[_state], FRAME_ROW_BY_DIRECTION[_direction])

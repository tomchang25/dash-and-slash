...
| 項目             | v0.4                                       | v0.5                                                                                                                           |
| ---------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| 玩家操作         | 即時自由移動，不佔 grid                    | Grid actor，佔 1 格，任何輸入 = 世界前進 1 tick                                                                                |
...
| Reward 架構      | Major ≤4 / Minor 無限 / store 投影         | 原封沿用，新增 speed 三通道                                                                                                    |
...
任何玩家輸入（移動 / 普攻 / 機動技 / 等待）= 世界恰好前進 1 tick，沒有例外。移動滑鼠（瞄準）永遠不消耗時間；非法輸入軟性拒絕、不消耗 tick。「慢」不藏在成本乘數裡，而是攤開成顯性 windup 回合。這條契約讓「還有 N 回合」永遠等於「你的任何 N 個輸入」——telegraph 在數學上不可能說謊，而不是靠 UI 補救。
...
```txt
1. 玩家行動完整解算（移動 / 普攻 / 機動技 / 等待，各恰好 1 tick）
...
底層是 energy/cost 骨架（每個 actor 有 speed 欄位，行動照 energy 結算），但對玩家暴露的永遠只有 tick 一種貨幣。玩家速度拆成三個互不綁死、各自由 reward store 投影的欄位：

| 概念     | 實作                                            | Minor 疊加方式                             |
| -------- | ----------------------------------------------- | ------------------------------------------ |
| 移動速度 | 免費步充能條：每次行動充 X%，滿了下一步世界靜止 | 充能率提高，平滑無限疊，收益量化但可預測   |
| 攻擊速度 | windup ticks（整數）                            | 疊到門檻 −1 windup（下限 0），或同走充能條 |
| 機動技   | 冷卻 ticks（整數）                              | −1 冷卻                                    |

敵人快慢用同一套骨架：慢怪每 2 tick 一動、快怪一 tick 兩動，意圖 icon 明示行動節奏。硬規則：**敵人雙動（玩家被減速側）必須視覺大聲**——兩段式預覽箭頭 + 專屬音效。加速自己是爽感，敵人加速是威脅，威脅側的可讀性投資加倍。

放棄的東西（有意識的交易）：連續值攻速縮放（沒有「+7% 攻速」，只有充能率或整數門檻）。在「模擬豐富」和「telegraph 永不說謊」之間，本作永遠選後者。

### 3.4 Windup（顯性前隙）

重動作 = 顯性 windup：按下起手（1 tick，自身 windup telegraph 亮起、目標格亮起）→ 下一個輸入釋放；改按其他動詞 = 取消起手，已花的 tick 不退款——維持「每個輸入恰好 1 tick、按什麼做什麼」的零特例原則。連發輸入整合：按住 = 連發脈衝第一下起手、第二下釋放，「按住 = 兩拍一擊」自然成立。
...
| 插槽                    | 按鍵                           | 滑鼠的作用                                  | 消耗   |
| ----------------------- | ------------------------------ | ------------------------------------------- | ------ |
| 移動                    | WASD 點按 / 按住連走（約 7Hz） | 無                                          | 1 tick |
| 普攻                    | 左鍵                           | 決定攻擊方向（量化到 4 向，武器形狀跟著轉） | 1 tick |
| 機動槽（Dash 或 Smash） | 右鍵 / Shift                   | 決定落點                                    | 1 tick |
| 等待                    | Space                          | 無                                          | 1 tick |
...
沿用 v0.4 §7 分層。新增：speed 三通道 Minor（§3.3）；Major 可以改時間規則本身（例：「dash 不消耗世界時間」= dash 變 free action）。Chain Dash 與其他追加 Major 是 rework 完成後的後續內容，不在轉換範圍。
...
隔離場景、灰方塊、兩種敵人（慢轉速近戰 + 2-tick charger）、speed 全鎖 baseline。kill criteria：「引誘 charger、側移、繞背一刀」感覺是賺來的動作爽感，而非選單戰棋。細節與驗收標準見 `dev/docs/plans/tick_combat_prototype.md`。閘門不過 = 停損，產線程式一行未動。
...
3. Speed 三通道 Minor 可疊，且任何 telegraph 顯示與實際解算零偏差
...
| 術語                    | 說明                                                                           |
| ----------------------- | ------------------------------------------------------------------------------ |
| Player-Clocked Tick     | 玩家時鐘制：世界只在玩家執行動詞時前進，一個輸入恰好一 tick                    |
| Tick                    | 一次玩家輸入推進的世界時間單位；全遊戲唯一對玩家暴露的時間貨幣                 |
| 三階段解算              | 玩家行動 → 歸零攻擊引爆（對新位置）→ 敵人移動與新 telegraph                    |
| Windup                  | 顯性前隙：起手佔 1 tick 並亮出自身 telegraph，下一輸入釋放，改按他鍵取消不退款 |
| 免費步（Free Step）     | 移動速度的量化收益：充能滿時下一步不推進世界                                   |
